...
void loadSong(int choice) {
  ...
  if (choice == 1) { // Twinkle Twinkle
    ...
    for(int i=0; i<currentSongLength; i++) {
      ...
      currentSong[i] = { (unsigned long)(startOffset + (i * 300)), mel[i], random(1, 5), false };
    }
  } 
    ...
    for(int i=0; i<currentSongLength; i++) {
      currentSong[i] = { (unsigned long)(startOffset + (i * 200)), mel[i], random(1, 5), false };
    }
  }
}
